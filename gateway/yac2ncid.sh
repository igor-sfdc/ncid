...
## Last changed on Oct 12, 2016 by jlc
##
## Call Format Example: @CALLBush George W.~(212) 555-9384\0
## Message Example:     Meet me for dinner at 8.\0
##
VERSION="(NCID) XxXxX"
VERBOSE=
...
COMMAND="HELLO: CMD: no_log"
IDENT="HELLO: IDENT: gateway yac2ncid $VERSION"
...
usage(){
    echo "Usage: yac2ncid [-v][-p <PIDFILE>]"
    echo "       yac2ncid [-V]"
}

while getopts ":p:vV" opt ; do
    case $opt in
    p)
      PIDFILE=$OPTARG
      ;;
...
then
	...
	then	
		...
		NETCAT_TYPE="BUSYBOX"
	elif ! isblank "$(${NETCAT:-nc} -h 2>&1 | egrep '[destination] [port]')"
	then

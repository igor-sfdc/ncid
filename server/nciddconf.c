...
int errorStatus, obsolete, inQuotes;
...
struct setword setword[] = {
    ...
    {"announce",   WORDSTR,            &announce,  0,          0,    0},
    {"audiofmt",   WORDSTR,            &audiofmt,  0,          0,    0},
    ...
    {"hupmode",    WORDNUM,            0,          &hupmode,   0,    3},
    {"hupname",    WORDSTR,            &hupname,   0,          0,    0},
    {"huprmd",     WORDSTR,            &huprmd,    0,          0,    0},
    {"gencid",     WORDNUM,            0,          &gencid,    OFF, ON},
    {"hangup",     WORDNUM,            0,          &hangup,    0,    3},
...
int doConf()
{
    char input[BUFSIZ], word[BUFSIZ], buf[BUFSIZ], *inptr, *ptr;
    int lc;
    FILE *fp;
    ...
    for (lc = 1; fgets(input, BUFSIZ, fp) != NULL; lc++)
    {
        ...
        if (inptr == 0 || input[0] == '\r' || word[0] == '#') continue;
    ...
    {
        obsolete = 0;
        sprintf(buf, "Warning: 'send outfile' is obsolete, upgrade %s file\n", cidconf);
        logMsg(LEVEL1, buf);
    }
...
void doSet(char *inptr, int lc)
{
    ...
    while ((inptr = getWord(fnptr, inptr, word, lc)))
    {
        ...
        {
            sprintf(buf, "Option: '%s' value in '%s' replaced by command line option\n",
...
char *getWord(char *nameptr, char *inptr, char *wdptr, int lc)
{
    ...
    *wdptr = 0;
    ...
    if (*inptr == '"')
    {
        ...
        /* inQuotes indicates the word is inside "" */
        inQuotes = 1;
    }
